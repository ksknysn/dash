...
export class ByEmployeeTypeComponent {
  chartType = signal<string>(
    localStorage.getItem('byemployeetypecomponent') || 'bar_chart'
  );
  ...
  constructor(
    ...
    effect(() => {
      this.drawChart();
      localStorage.setItem('byemployeetypecomponent', this.chartType() || 'bar_chart');  
    });
  }
  ...
  ngOnInit(): void {
    this.httpClientService
      .get<ByEmployeeTypeResponse>({ controller: 'totalResourcesByEmploymentType' })
      .subscribe((response) => {
        // Gelen veriyi grafik için uygun formata çeviriyoruz
        this.data = Object.entries(response.total_resources_by_employment_type).map(
          ([key, value]) => ({
            label: key,
            value: value
          })
...
interface ByEmployeeTypeResponse {
  total_resources_by_employment_type: {
