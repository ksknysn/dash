...
export class ByRaceDescComponent {
  ...
  ngOnInit(): void {
    this.httpClientService
      .get<ByRaceDescResponse>({ controller: 'totalResourcesByRaceDesc' })
      .subscribe((response) => {
        // Gelen veriyi grafik için uygun formata çeviriyoruz
        this.data = Object.entries(response.total_resources_by_race_desc).map(
          ([key, value]) => ({
            label: key,
            value: value
          })
...
interface ByRaceDescResponse {
  total_resources_by_race_desc: {
