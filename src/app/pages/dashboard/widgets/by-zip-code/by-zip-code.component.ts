...
export class ByZipCodeComponent {
  ...
  ngOnInit(): void {
    this.httpClientService
      .get<ByZipCodeResponse>({ controller: 'totalResourcesByZipCode' })
      .subscribe((response) => {
        // Gelen veriyi grafik için uygun formata çeviriyoruz
        this.data = Object.entries(response.total_resources_by_zip_code).map(
          ([key, value]) => ({
            label: key,
            value: value
          })
...
interface ByZipCodeResponse {
  total_resources_by_zip_code: {
