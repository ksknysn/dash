...
@Component({
  selector: 'app-by-employment-status',
  standalone: true,
...
export class ByEmploymentStatusComponent {
  chartType = signal<string>(
    localStorage.getItem('byemploymentstatuscomponent') || 'bar_chart'
  );
  ...
  constructor(
    ...
    effect(() => {
      this.drawChart();
      localStorage.setItem('byemploymentstatuscomponent', this.chartType() || 'bar_chart');  
    });
  }
  ...
  ngOnInit(): void {
    this.httpClientService
      .get<ByEmployementStatusResponse>({ controller: 'totalResourcesByEmploymentStatus' })
      .subscribe((response) => {
        // Gelen veriyi grafik için uygun formata çeviriyoruz
        this.data = Object.entries(response.total_resources_by_employment_status).map(
          ([key, value]) => ({
            label: key,
            value: value
          })
...
interface ByEmployementStatusResponse {
  total_resources_by_employment_status: {
