...
export class ByStateComponent {
  chartType = signal<string>(
    localStorage.getItem('bystatecomponent') || 'bar_chart'
  );
  ...
  constructor(
    ...
    effect(() => {
      this.drawChart();
      localStorage.setItem('bystatecomponent', this.chartType() || 'bar_chart');  
    });
  }
  ...
  ngOnInit(): void {
    this.httpClientService
      .get<ByStateResponse>({ controller: 'totalResourcesByState' })
      .subscribe((response) => {
        // Gelen veriyi grafik için uygun formata çeviriyoruz
        this.data = Object.entries(response.total_resources_by_state).map(
          ([key, value]) => ({
            label: key,
            value: value
          })
...
interface ByStateResponse {
  total_resources_by_state: {
