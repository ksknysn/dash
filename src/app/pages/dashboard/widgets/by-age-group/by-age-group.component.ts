...
export class ByAgeGroupComponent {
  chartType = signal<string>(
    localStorage.getItem('byagegroupcomponent') || 'bar_chart'
  );
  ...
  constructor(
    ...
    effect(() => {
      this.drawChart();
      localStorage.setItem('byagegroupcomponent', this.chartType() || 'bar_chart');  
    });
  }
  ...
  ngOnInit(): void {
    this.httpClientService
      .get<ByAgeGroupResponse>({ controller: 'totalResourcesByAgeGroup' })
      .subscribe((response) => {
        // Gelen veriyi grafik için uygun formata çeviriyoruz
        this.data = Object.entries(response.total_resources_by_age_group).map(
          ([key, value]) => ({
            label: key,
            value: value
          })
...
interface ByAgeGroupResponse {
  total_resources_by_age_group: {
