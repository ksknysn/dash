...
@Component({
  selector: 'app-by-performance-score',
  standalone: true,
...
export class ByPerformanceScoreComponent {
  chartType = signal<string>(
    localStorage.getItem('bymaritalstatuscomponent') || 'bar_chart'
  );
  ...
  constructor(
    ...
    effect(() => {
      this.drawChart();
      localStorage.setItem('bymaritaltatuscomponent', this.chartType() || 'bar_chart');  
    });
  }
  ...
  ngOnInit(): void {
    this.httpClientService
      .get<ByPerformanceScoreResponse>({ controller: 'totalResourcesByPerformanceScore' })
      .subscribe((response) => {
        // Gelen veriyi grafik için uygun formata çeviriyoruz
        this.data = Object.entries(response.total_resources_by_performance_score).map(
          ([key, value]) => ({
            label: key,
            value: value
          })
...
interface ByPerformanceScoreResponse {
  total_resources_by_performance_score: {
