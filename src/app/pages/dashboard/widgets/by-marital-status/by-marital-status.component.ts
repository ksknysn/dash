...
@Component({
  selector: 'app-by-marital-status',
  standalone: true,
...
export class ByMaritalStatusComponent {
  ...
  ngOnInit(): void {
    this.httpClientService
      .get<ByMaritalStatusResponse>({ controller: 'totalResourcesByMaritalStatus' })
      .subscribe((response) => {
        // Gelen veriyi grafik için uygun formata çeviriyoruz
        this.data = Object.entries(response.total_resources_by_marital_status).map(
          ([key, value]) => ({
            label: key,
            value: value
          })
...
interface ByMaritalStatusResponse {
  total_resources_by_marital_status: {
